...
assert.match(source, /url\.pathname === "\/admin\/access"/);
...
assert.match(source, /headers\["referrer-policy"\] = "same-origin"/);
...
assert.match(source, /constantTimeEqual\(suppliedHash, expectedHash\)/);
assert.match(source, /admin-access-rate:/);
assert.match(source, /hmacHex\(env\.ADMIN_TOKEN, `admin-access\\n\$\{address\}`\)/);
assert.match(source, /ADMIN_ACCESS_MAX_FAILURES = 5/);
assert.match(source, /attempts >= ADMIN_ACCESS_MAX_FAILURES[\s\S]{0,180}429/);
assert.match(source, /expirationTtl: ADMIN_ACCESS_RATE_TTL_SECONDS/);
assert.match(source, /authMethod: "admin_token"/);
assert.match(source, /setCookie\(ADMIN_SESSION_COOKIE, sessionToken, ADMIN_SESSION_TTL_SECONDS\)/);
assert.match(source, /clearCookie\(ADMIN_ACCESS_CSRF_COOKIE\)/);
assert.match(source, /管理コードで本人確認済み/);
assert.match(source, /href="\/admin\/login">GitHubログイン（予備）/);
assert.doesNotMatch(source, /href=["'][^"']*ADMIN_TOKEN/i);
assert.doesNotMatch(source, /name="adminToken"[^>]*value=/i);
assert.doesNotMatch(source, /admin-session:[\s\S]{0,500}adminToken\s*:/i);
assert.doesNotMatch(source, /console\.(?:log|error|warn)\([^\n]*(?:ADMIN_TOKEN|adminToken)/i);
assert.match(source, /apiRoute: "\/mcp"/);
assert.match(source, /authorizeEndpoint: "\/authorize"/);
assert.match(source, /github\.com\/login\/oauth\/authorize/);
...
assert.match(source, /return await handleAdminRequest\(request, env\)/);
assert.match(source, /return await saveMakeToken\(form, env\)/);
assert.match(source, /return await deleteMakeToken\(form, env\)/);
assert.match(source, /return await runMakeDiagnosis\(env\)/);
assert.match(source, /return await renderMakeAdmin\(request, env, session,[\s\S]{0,240}catch \{[\s\S]{0,180}renderAdminFailure/);
assert.match(source, /return await appHandler\.fetch\(request, env, ctx\)/);
assert.match(source, /function renderAdminFailure\(message, returnTo\)/);
